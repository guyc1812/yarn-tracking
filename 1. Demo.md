# YARN 任务跟踪探明 - 准备一块敲门砖: Word Count Demo

## **Demo 准备, 书写并提交一个 Spark 应用**

本篇将准备一个 Hello World 程序, 作为整个系列的演示, Demo 程序将完成以下动作：
1. 对接一个 Hadoop 集群
...
### **项目准备**
   ...
   * 静态文件资源：
      * 本地 Jar 包
         1. src/test/resources/jar-files/word-count-demo.jar, 项目编译好之后, copy 到 resources 目录下
         2. src/test/resources/jar-files/gson-2.8.0.jar, 第三方测试包
      * 本地配置文件
...
1. 准备 Word Count 示例程序, 程序中打印 Spark 应用信息和系统变量
    ...
    ```
2. 准备 Spark 任务提交程序, 用于接收 Spark 应用的配置和参数, 完成提交应用到 YARN 的动作
    ```java
    package com._4pd.spark;
...
4. 运行
    ```bash
    19/10/27 16:16:17 INFO SparkSubmitter: Application: application_1571091072053_54110 submitted, tracking url is http://m7-common-cdh07:8088/proxy/application_1571091072053_54110/.
    ```


### ***Demo 涉及到的 Spark 配置, 按上例使用顺序***
...
下一篇将介绍提交任务的 Spark Client 到底做了什么。
